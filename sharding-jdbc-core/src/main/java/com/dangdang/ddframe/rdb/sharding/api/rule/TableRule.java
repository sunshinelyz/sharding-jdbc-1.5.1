...
public final class TableRule {
    ...
    private final boolean dynamic;//是否动态分片
    
    private final List<DataNode> actualTables;//根据分片规则解析完的每个要执行的单元，这个单元是DataNode，例如01库.08月份表,01库.09月份表
    
    private final DatabaseShardingStrategy databaseShardingStrategy;//数据库分片策略解析实现
    
    private final TableShardingStrategy tableShardingStrategy;//表分片策略解析实现
    ...
    private List<DataNode> generateDataNodes(final List<String> actualTables, final DataSourceRule dataSourceRule, final Collection<String> actualDataSourceNames) {
        ...
        for (String actualTable : actualTables) {
            if (DataNode.isValidDataNode(actualTable)) {
                result.add(new DataNode(actualTable));//组装DataNode
            } else {
...
//        获取真实的数据单元
    ...
    public static class TableRuleBuilder {
        ...
        private List<String> actualTables;//物理表集合
        
        private DataSourceRule dataSourceRule;//数据源规则配置对象
        
        private Collection<String> dataSourceNames;//数据源名称集合
        
        private DatabaseShardingStrategy databaseShardingStrategy;//数据库分片配置解析对象
        
        private TableShardingStrategy tableShardingStrategy;//表分片配置解析对象
        ...
        public TableRule build() {
            ...
            return new TableRule(logicTable, dynamic, actualTables, dataSourceRule, dataSourceNames, databaseShardingStrategy, tableShardingStrategy, generateKeyColumn, keyGenerator);//分表规则解析
        }
    }
}
