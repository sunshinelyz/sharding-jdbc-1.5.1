...
public class Lexer {
    ...
    private int offset;//记录当前token的offset
    
    @Getter
    private Token currentToken;//解析完每一个最小单元都是一个token，如select * from 解析完成后就是 select、*、from 三个token
    
    /**
     * 分析下一个词法标记.
     */
    public final void nextToken() {//词法解析就是调用这个方法把sql语句解析成一个一个的token，业务方法
        skipIgnoredToken();//过滤掉无用的token
        if (isVariableBegin()) {
            currentToken = new Tokenizer(input, dictionary, offset).scanVariable();//这里的dictionary是具体的数据库关键字字典
    ...
    private void skipIgnoredToken() {
        offset = new Tokenizer(input, dictionary, offset).skipWhitespace();//先过滤掉sql语句中的空格
        while (isHintBegin()) {
            offset = new Tokenizer(input, dictionary, offset).skipHint();
            offset = new Tokenizer(input, dictionary, offset).skipWhitespace();
        }
        while (isCommentBegin()) { //判断是不是以注释开头，过滤掉sql语句中注释这些token
