...
public final class Tokenizer {
    ...
    public int skipComment() {
        ...
        if (isSingleLineCommentBegin(current, next)) {//是否是//，--这样的换行
            return skipSingleLineComment(COMMENT_BEGIN_SYMBOL_LENGTH);
        } else if ('#' == current) { //是否是#这样的换行
            return skipSingleLineComment(MYSQL_SPECIAL_COMMENT_BEGIN_SYMBOL_LENGTH);
        } else if (isMultipleLineCommentBegin(current, next)) {//是否是多行注释
    ...
    public Token scanSymbol() {
        int length = 0;
        while (CharType.isSymbol(charAt(offset + length))) {//判断是不是符号
            length++;
        }
        String literals = input.substring(offset, offset + length);
        Symbol symbol;//符号集合枚举类
        while (null == (symbol = Symbol.literalsOf(literals))) {
            literals = input.substring(offset, offset + --length);
        }
        return new Token(symbol, literals, offset + length);//最后返回一个token
    }
