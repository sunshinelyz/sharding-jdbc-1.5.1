...
public abstract class AbstractSQLParser extends AbstractParser {
    ...
    public final SQLExpression parseExpression() {//解析表达式
...
//        别名之前必须要有as关键字的，否则解析不到
    ...
    public final void parseSingleTable(final SQLStatement sqlStatement) {
        ...
        if (skipIfEqual(Symbol.LEFT_PAREN)) {
            if (equalAny(DefaultKeyword.SELECT)) {//不支持子查询 insert into select，不支持delete from select，
        ...
        if (skipJoin()) {//不支持insert into，delete from 后面跟表关联
...
//            解析where后的条件表达式
    ...
    private void parseConditions(final SQLStatement sqlStatement) {
        do {
            parseComparisonCondition(sqlStatement);//解析比较表达式
...
//        解析between表达式
    ...
    private void parseRowNumberCondition(final SelectStatement selectStatement) {//解析oracle分页
